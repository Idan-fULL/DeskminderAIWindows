        ...
        public static void SetStartupWithWindows(bool enable)
        {
            try
            {
                using (RegistryKey key = Registry.CurrentUser.OpenSubKey(RUN_REGISTRY_KEY, true))
                {
                    if (key == null)
                    {
                        WPFMessageBox.Show("Failed to access startup registry key.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                        return;
                    }
            ...
            {
                WPFMessageBox.Show($"Failed to set startup preference: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        ...
        private void ReminderContainer_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                if (sender is Grid grid)
                {
                    ...
                    var deleteButton = FindChildByName<Button>(grid, "DeleteButton");
                    var reminderText = parentGrid.Children.OfType<Border>().FirstOrDefault(b => b.Name == "ReminderText");
                    var minutesBubble = parentGrid.Children.OfType<Border>().FirstOrDefault(b => b.Name == "MinutesBubble");

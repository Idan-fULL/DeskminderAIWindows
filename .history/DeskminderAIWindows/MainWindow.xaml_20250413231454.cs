...
namespace DeskminderAI
{
    public partial class MainWindow : Window
    {
        ...
        private void TaskbarIcon_TrayLeftMouseDown(object sender, RoutedEventArgs e)
        {
            try
            {
                Console.WriteLine("TaskbarIcon_TrayLeftMouseDown event triggered");
                ...
                if (ViewModel.Reminders.Count > 0)
                {
                    ...
                    Console.WriteLine("Showing active reminders");
                }
                else
                {
                    ...
                    ChangeVisualState(STATE_ICON_ONLY);
                    ResizeWindowForState(STATE_ICON_ONLY);
                    Console.WriteLine("Showing icon-only state");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error in TaskbarIcon_TrayLeftMouseDown: {ex.Message}");
        ...
        private void TimerDragCanvas_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                if (_isDraggingDuration && TimerDragCanvas != null)
                {
                    ...
                    Console.WriteLine($"Duration drag at position {currentX}, minutes: {newMinutes}");

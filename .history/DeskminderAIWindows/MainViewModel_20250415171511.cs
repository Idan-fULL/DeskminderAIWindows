...
namespace DeskminderAI
{
    public class Reminder : INotifyPropertyChanged
    {
        ...
        public Reminder(string name, int minutes)
        {
            ...
            // Set end time to exact minute count from now
            // Rounding down to the nearest second to avoid extra time
            DateTime now = DateTime.Now;
            EndTime = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, 0).AddMinutes(minutes);
        ...
        public void UpdateTimeLeft()
        {
            ...
            // Format as minutes:seconds with no leading zeros for minutes
            int minutes = (int)Math.Floor(timeLeft.TotalMinutes);
            int seconds = (int)Math.Floor(timeLeft.TotalSeconds) % 60;
            
            // Use format with minutes without leading zero followed by seconds with leading zero
            TimeLeftDisplay = $"{minutes}:{seconds:00}";
            
            // Update DisplayTime to show minutes only
            DisplayTime = $"{minutes}";
        }
    ...
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        public void SaveReminders()
        {
            try
            {
                ...
                var json = JsonSerializer.Serialize(reminderData, options);

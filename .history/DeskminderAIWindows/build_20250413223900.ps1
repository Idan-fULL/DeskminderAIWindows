...
# Determine .NET version and set output directory accordingly
$netVersion = "net10.0-windows"
$binDir = Join-Path $projectDir "bin"
$configDir = Join-Path $binDir $buildConfig
$outputDir = Join-Path $configDir $netVersion
...
try {
    ...
    Write-Host "Failed to build using dotnet. Trying MSBuild directly..."
    
    # Try to find MSBuild
    $msbuildPath = "C:\Program Files\Microsoft Visual Studio\2022\Community\MSBuild\Current\Bin\MSBuild.exe"
    
    if (Test-Path $msbuildPath) {
        & $msbuildPath $slnPath /p:Configuration=$buildConfig
        ...
        Write-Host "Build completed successfully using MSBuild"
    }
    else {
        Write-Host "Could not find MSBuild. Please build manually using Visual Studio."
        exit 1
    }
}

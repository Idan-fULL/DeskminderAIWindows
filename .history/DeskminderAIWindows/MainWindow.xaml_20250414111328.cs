    ...
    public partial class MainWindow : Window
    {
        ...
        private void LoadExistingReminders()
        {
            ...
            if (ViewModel.Reminders.Count > 0)
            {
                ...
                // They'll be shown on MouseLeave events
                ListReminders.UpdateLayout();
                foreach (var item in ListReminders.Items)
                {
                    var container = ListReminders.ItemContainerGenerator.ContainerFromItem(item) as FrameworkElement;
                    if (container != null)
                    {
                        var grid = FindChildByName<Grid>(container, "ReminderContainer");
                        if (grid != null)
                        {
                            var minutesBubble = FindChildByName<Border>(grid, "MinutesBubble");
                            if (minutesBubble != null)
                            {
                                minutesBubble.Visibility = Visibility.Visible;
                            }
        ...
        private void ShowTimerPicker()
        {
            try
            {
                ...
                // Move the TimerSelectionDisplay to the center of the screen
                // First, ensure the window is visible
                EnsureWindowVisibility();
                
                // Position the timer element in the center of the screen regardless of window position
                ...
                // Measure the size of the TimerSelectionDisplay
                TimerSelectionDisplay.Visibility = Visibility.Visible;
                TimerSelectionDisplay.Measure(new System.Windows.Size(double.PositiveInfinity, double.PositiveInfinity));
                double timerWidth = TimerSelectionDisplay.DesiredSize.Width;
                double timerHeight = TimerSelectionDisplay.DesiredSize.Height;
                
                // Calculate the position to center the timer on screen
                double leftPosition = (screenWidth - timerWidth) / 2;
                double topPosition = (screenHeight - timerHeight) / 2;
                
                // Convert screen coordinates to window coordinates
                Point screenPoint = new Point(leftPosition, topPosition);
                Point windowPoint = this.PointFromScreen(screenPoint);
                
                // Set the margin to position the timer
                ...
                Console.WriteLine($"Timer picker shown at screen position: X={leftPosition}, Y={topPosition}");
            }
            catch (Exception ex)
            {
        ...
        private void ReminderContainer_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                if (sender is Grid grid)
                {
                    ...
                    if (reminderText != null && minutesBubble != null)
                    {
                        // Show the reminder text and hide minutes bubble
                        reminderText.Visibility = Visibility.Visible;
                        minutesBubble.Visibility = Visibility.Collapsed;
                        
                        Console.WriteLine($"MouseEnter: Showing ReminderText, hiding MinutesBubble for {grid.Name}");
                    }
                    else
                    {
                        Console.WriteLine($"MouseEnter: Could not find ReminderText or MinutesBubble for {grid.Name}");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void ReminderContainer_MouseLeave(object sender, MouseEventArgs e)
        {
            try
            {
                if (sender is Grid grid)
                {
                    ...
                    if (reminderText != null && minutesBubble != null)
                    {
                        // Hide the reminder text and show minutes bubble
                        reminderText.Visibility = Visibility.Collapsed;
                        minutesBubble.Visibility = Visibility.Visible;
                        
                        Console.WriteLine($"MouseLeave: Hiding ReminderText, showing MinutesBubble for {grid.Name}");
                    }
                    else
                    {
                        Console.WriteLine($"MouseLeave: Could not find ReminderText or MinutesBubble for {grid.Name}");
                    }
                }
            }
            catch (Exception ex)

    ...
    public partial class MainWindow : Window
    {
        ...
        private void FloatingAddButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Toggle to full UI mode first, then show the add reminder interface
                OpenTimeSettingMenu();
            }
            catch (Exception ex)
            {
        ...
        private async void OpenTimeSettingMenu()
        {
            try
            {
                // First, just switch to the full UI mode
                SetIconOnlyMode(false);
                
                // Let the UI render before trying to show the add reminder panel
                await Task.Delay(100);
                
                // Now show the add reminder panel
                try
                {
                    var vm = ViewModel;
                    if (vm?.ShowAddReminderCommand != null)
                    {
                        vm.ShowAddReminderCommand.Execute(null);
                    }
                    else
                    {
                        // If we can't get the view model or command, at least show the UI
                        Console.WriteLine("ShowAddReminderCommand was null, but UI is shown");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error showing add reminder panel: {ex.Message}");
                    // Continue showing the UI even if we can't show the add reminder panel
                }
            }
            catch (Exception ex)
            {

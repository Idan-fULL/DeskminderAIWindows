    ...
    public partial class MainWindow : Window
    {
        ...
        private void ReminderContainer_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                {
                    // Get the parent grid that contains all elements
                    var parentGrid = VisualTreeHelper.GetParent(grid) as Grid;
                    if (parentGrid == null) return;

                    // Find the delete button within the reminder container grid
                    var deleteButton = FindElementByName<Button>(grid, "DeleteButton");
                    // Find bubble elements in the parent grid
                    var reminderText = FindElementByName<Border>(parentGrid, "ReminderText");
                    var minutesBubble = FindElementByName<Border>(parentGrid, "MinutesBubble");
        ...
        private T FindElementByName<T>(FrameworkElement container, string name) where T : FrameworkElement
        {
            ...
            var element = container.FindName(name) as T;
            if (element != null)
                return element;

...
namespace DeskminderAI
{
    public partial class MainWindow : Window
    {
        ...
        private Dictionary<UIElement, Point> _originalReminderPositions = new Dictionary<UIElement, Point>();
        ...
        private void ReminderContainer_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                if (sender is Grid grid)
                {
                    ...
                    // Get the size of the window and the element's position
                    double screenWidth = SystemParameters.WorkArea.Width;
                    double elementRight = grid.TransformToAncestor(this).Transform(new WinPoint(grid.ActualWidth, 0)).X;
                    bool isNearRightEdge = (screenWidth - elementRight) < 200; // 200px threshold
                    
                    // Don't show expanded elements if near the edge to prevent overflow
                    if (isNearRightEdge)
                    {
                        // Just show the countdown timer without expansion
                        if (reminderText != null) reminderText.Visibility = Visibility.Collapsed;
                        if (minutesBubble != null) minutesBubble.Visibility = Visibility.Collapsed;
                        Console.WriteLine($"MouseEnter: Near edge, not showing expanded elements for {grid.Name}");
                    }
                    else
                    {
                        // Show the reminder text when not near edge
                        if (reminderText != null)
                        {
                        ...
                        {
                            minutesBubble.Visibility = Visibility.Collapsed;
                            Console.WriteLine($"MouseEnter: MinutesBubble kept Collapsed for {grid.Name}");
                        }
                    }
                }
            }
        ...
        private void CollapseRemindersWithAnimation()
        {
            try
            {
                ...
                foreach (ContentPresenter presenter in ListReminders.ItemsPanel.FindName("ListReminders") as ItemsControl)
                {
                    var container = VisualTreeHelper.GetChild(presenter, 0) as FrameworkElement;
                    if (container != null)
                    {
                        // Find the reminder container in the visual tree
                        var reminderContainer = FindChildByName<Grid>(container, "ReminderContainer");
                        if (reminderContainer != null)
                        {
                            reminderElements.Add(reminderContainer);
                            
                            // Store original position for later restoration
                            Point originalPos = reminderContainer.TranslatePoint(new Point(0, 0), this);
                ...
                Point targetPoint = AddButtonOnly.TranslatePoint(new Point(24, 24), this);
                
                // Create and start animations for each reminder
                foreach (UIElement reminder in reminderElements)
                {
                    Point startPoint = reminder.TranslatePoint(new Point(0, 0), this);
        ...
        private void ExpandRemindersWithAnimation()
        {
            try
            {
                ...
                // Get all reminder containers
                foreach (ContentPresenter presenter in ListReminders.ItemContainerGenerator.Items)
                {
                    var container = VisualTreeHelper.GetChild(presenter, 0) as FrameworkElement;
                    if (container != null)
                    {
                ...
                Point startPoint = AddButtonOnly.TranslatePoint(new Point(24, 24), this);

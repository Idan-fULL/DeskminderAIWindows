    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Get settings from application
                if (Application.Current is App app)
                {
                    // Get MainViewModel from resources
                    if (Application.Current.Resources["MainViewModel"] is MainViewModel mainViewModel)
                    {
                ...
                // NOTE: Removed automatic timer selection window display
                // Allow the window to display and stabilize properly
        ...
        private void ExportReminders_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Get the reminders from the view model
                var reminders = ViewModel.Reminders.ToList();
                
                // Check if there are any reminders to export
                if (reminders == null || !reminders.Any())
                {
                    MessageBox.Show("אין תזכורות לייצוא.", "אין נתונים", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                var dialog = new Microsoft.Win32.SaveFileDialog
                {
                    ...
                    FileName = $"Deskminder_Reminders_{DateTime.Now:yyyy-MM-dd}"
                };

                if (dialog.ShowDialog() == true)
                {
                    bool success = DeskminderAI.Services.ReminderService.ExportReminders(reminders, dialog.FileName);
                    if (success)
                    {
                        MessageBox.Show($"יוצאו בהצלחה {reminders.Count} תזכורות אל {dialog.FileName}", 
                            "הייצוא הושלם", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                    else
                    {
                        MessageBox.Show("ייצוא התזכורות נכשל. אנא נסה שנית.", 
                            "הייצוא נכשל", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"אירעה שגיאה: {ex.Message}", "שגיאת ייצוא", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

    ...
    public partial class MainWindow : Window
    {
        ...
        private void ExportReminders_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // If user selected a file path, export the reminders
                if (result == true)
                {
                    string filePath = saveFileDialog.FileName;
                    bool success = ReminderService.ExportReminders(reminders.ToList(), filePath);
                    
                    if (success)
                    {
                        MessageBox.Show("התזכורות יוצאו בהצלחה", "ייצוא הושלם", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                    else
                    {
                        MessageBox.Show("שגיאה בייצוא התזכורות", "שגיאת ייצוא", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
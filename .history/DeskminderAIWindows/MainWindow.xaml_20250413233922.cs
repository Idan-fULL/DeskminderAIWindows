    ...
    public partial class MainWindow : Window
    {
        ...
        private void TaskbarIcon_TrayLeftMouseDown(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // If we're in icon-only state, expand to timer state
                var currentState = VisualStateManager.GetVisualStateGroups((FrameworkElement)Content)
                    ?.FirstOrDefault()
                    ?.CurrentState
                    ?.Name;
                    
                if (currentState == STATE_ICON_ONLY)
                {

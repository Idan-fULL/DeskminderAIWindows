...
namespace DeskminderAI
{
    public partial class MainWindow : Window
    {
        ...
        private void CreateAndSetTaskbarIcon()
        {
            try
            {
                // Create a green circle icon programmatically
                using (Bitmap bitmap = new Bitmap(32, 32))
                {
                    ...
                    using (MemoryStream ms = new MemoryStream())
                    {
                        ...
                        if (TaskbarIcon != null)
                        {
                            // Create a DrawingImage from the BitmapImage
                            DrawingImage drawingImage = new DrawingImage(
                                new ImageDrawing(bitmapImage, new Rect(0, 0, 32, 32)));
                            
                            // Set the icon
                            TaskbarIcon.Icon = drawingImage;

...
namespace DeskminderAI
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        {
            // Update all reminders first
            foreach (var reminder in Reminders.ToList())
            {
                // Make sure time left is updated correctly
                reminder.UpdateTimeLeft();
                
                // No need to manually call OnPropertyChanged, the UpdateTimeLeft method already does this
            }
            
            // Then remove expired reminders in a separate loop
            foreach (var reminder in Reminders.ToList())
            {
        ...
        public void SaveReminders()
        {
            try
            {
                ...
                var json = JsonConvert.SerializeObject(Reminders.ToArray(), Formatting.Indented);
        ...
        public void LoadReminders()
        {
            try
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    var savedReminders = JsonConvert.DeserializeObject<Reminder[]>(json);
